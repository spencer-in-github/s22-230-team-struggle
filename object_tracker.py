# modified based on https://github.com/theAIGuysCode/yolov4-custom-functions/blob/master/detect_video.py
import os
...
def main(_argv):
    ...
    # calculate cosine distance metric -> to incorporate the uncertainties from the Kalman filter
    ...
    while True:
        ...
        if FLAGS.framework == 'tflite':
            ...
            pred_bbox = infer(batch_data) # running inference from the saved model
            for key, value in pred_bbox.items(): # includes the bounding box and the confidence level
                boxes = value[:, :, 0:4]
                pred_conf = value[:, :, 4:]
        
        # greedily selecta subset of bounding boxes in descending order of score
        ...
        if FLAGS.count:
            ...
            for i, (key, value) in enumerate(counted_classes.items()):
                print("Testing: Number of {}s: {}".format(key, value))
                cv2.putText(frame, "{} being tracked: {}".format(key, value), (5, 60+i*16), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 0), 2)
            cv2.putText(frame, "Objects being tracked: {}".format(count), (5, 40), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2, (0, 255, 0), 2)
